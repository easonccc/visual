<template>
  <div :style="{ height: height, width: width }" id="mapBoxPeople"></div>
</template>
<script>
...
import people from "../../assets/img/abry.png";
export default {
  props: {
    type: {
      type: String,
  ...
  methods: {
    ...
    getAll() {
      let that = this;
      http.get("./data/dataVisitor2.json").then(v => {
        let data = v.data;
        data.forEach(v => {
    ...
    drawPoint(device) {
      ...
      var that = this;
      that.map = new AMap.Map("mapBoxPeople", {
        resizeEnable: true,
